...
Page({
  ...
  loadDetail(sourceType, id) {
    ...
    request.get(apiUrl).then((data) => {
      ...
      const isAiStyle = this.data.isAiStyle || recordSourceType.includes('AI') || !!(data.aiStyle || data.originalImageUrl || data.inputImageUrl);
      const originalUrl = (sourceType === 'DRAFT') ? '' : (data.originalImageUrl || data.inputImageUrl || data.sourceUrl || data.coverUrl || '');
  ...
  onConfirmSavePattern() {
    const { patternNameInput, historyId, draftId, sourceType, mappedPixelData, currentSize, brandName, colorCount, originalUrl, aiStyleTag, isAiStyle } = this.data;
